### **Guide: Installing XmdS on RevyOS**

This document provides instructions for installing the `XmdS` (a tool for solving differential equations) on a RISC-V device running the RevyOS Debian derivative. Since the `xmds2` package is available in the RevyOS repositories, the installation process will be very straightforward.
...
Running the generated program produces:
...
This demonstrates that `xmds2` is basically functional on RevyOS.
