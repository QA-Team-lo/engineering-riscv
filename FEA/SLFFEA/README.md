### **Guide: Building SLFFEA on RevyOS**

This document provides instructions for building the `slffea` software from source on a RISC-V device running RevyOS.
...
#### **Step 2: Install Dependencies and Modify Makefile**

* Install the required dependencies (including GUI support) with apt:

```bash
$ sudo apt-get install -y libx11-dev libxext-dev mesa-common-dev libglu1-mesa-dev \
  freeglut3-dev libxi-dev libxmu-dev libxt-dev libsm-dev libice-dev
```

* Modify all `Makefile`s to ensure `CFLAGS` are included:
...
Use one of the provided example datasets for testing:
...
This confirms that `slffea` has basic functionality on RevyOS.
